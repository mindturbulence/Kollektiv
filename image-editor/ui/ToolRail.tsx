...
import {
  MoveIcon, SquareDashedIcon, LassoIcon, WandIcon, CropIcon, BrushIcon, EraserIcon,
  GradientIcon, ShapeToolIcon, TypeToolIcon, EyedropperToolIcon, HandToolIcon, ZoomToolIcon,
...
const FgBgSwatches: React.FC = () => {
  ...
  return (
    <div className="relative w-11 h-11 flex-shrink-0 flex items-center justify-center">
      <button
        type="button"
        className="absolute top-1.5 left-1.5 w-5 h-5 rounded-sm border border-base-content/30 shadow-sm"
        style={{ backgroundColor: colors.background }}
        aria-label="Background color"
        onClick={() => alert('Color picker coming soon')}
      />
      <button
        type="button"
        className="absolute bottom-1.5 right-1.5 w-5 h-5 rounded-sm border border-base-content/30 shadow-sm"
