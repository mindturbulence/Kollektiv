...
- server.ts: the Express dev host / proxy bridge. Down to ~1,150 lines as of 2026-07-29 — route handlers were extracted into `routes/*.ts` (reach, search, MCP proxy, local model proxies, Topaz), one router per file, pure extraction with no behavior change. Note this landed at top-level `routes/`, not `src/routes/` as originally planned — `src/` still holds only `middleware/` and `schemas/`. See [Security Hardening](#security-hardening).
...
- [ ] Gallery intelligence: similarity clustering, visual search — **not shipped.** These were falsely claimed as done. Clustering depends on embeddings (Phase 5 of the expansion roadmap); visual search is a follow-on.
...
- **Reach channels — 6 content-reach capabilities (2026-07-26):** Added `rss_fetch`, `github_get_repo`/`github_search`/`github_get_file`, `exa_search`, `reddit_fetch`, `youtube_get_transcript`, and `twitter_get_tweet` tools. Each backed by a `POST /api/reach/<channel>` server route with Zod validation and rate limiters. Dual-backend ordered-fallback architecture for YouTube (watch-page → InnerTube) and Twitter (syndication CDN → oEmbed). 50+ new unit tests. Fragility documented per-channel (Twitter highest, YouTube elevated).
...
### Phase 5 — Memory Promotion, Inter-Step Data Flow, ComfyUI Import, Refiner Weighting, Server Split ✅

Five workstreams, code-reviewed and revised (v3.0.0) before implementation — two others (visual gallery search, a local extension/plugin hub) were gated out at the design stage rather than built; see below. Originally tracked in `docs/plans/2026-07-28-kollektiv-roadmap-and-improvements.md`; deleted 2026-07-29 once implemented and folded in here.

- [x] **Memory promotion wiring.** `memoryTierService.trackAccess()` (`services/memoryTierService.ts:176`) already existed, fully implemented with configurable thresholds — it just had zero callers, so access counts climbed forever without ever promoting a memory `working` → `long-term` → `knowledge`. Split into a pure policy check (increment stays owned by `knowledgeService.recall()` → `touchAccess()`) and wired at 4 real call sites: `knowledge_lifecycle_promote`, `get_gallery_item`, `buildKnowledgeContextBlock`'s search-result selection, and the memory-prompt-block injection path. That last one first shipped as a loop that called `trackAccess()` on *every* stored memory on every chat turn — which would have made every memory promote after 3 turns regardless of actual relevance, defeating the entire point of usage-based promotion — caught in review and deleted entirely rather than patched. 8 tests.
- [x] **Capability pipeline inter-step data flow.** Plan steps could execute but never consume each other's output. `executionEngine.ts` now tracks `stepOutputs` keyed by both `step1`/`step2`/… and array index (`step[0]`), and resolves `{{step1.output.summary}}`-style templates in a step's `params` before dispatch — exact-match templates return the raw value (preserving object/array types), embedded templates stringify. An unresolved template **fails the step** rather than silently substituting `""`. 19 new tests; all 18 pre-existing `executionEngine.test.ts` tests continue to pass unmodified. One inconsistency survived review and is left as a known follow-on: the *pre-existing* dispatch-failure path (from ISSUE-47) still unconditionally records a hard-failing non-optional step's own `StepResult.status` as `'skipped'`, while this new template-failure path correctly records `'failed'` — same semantic situation, two different individual step statuses depending on which code path triggered it. `capability_execute`'s JSON output surfaces this directly.
- [x] **ComfyUI custom workflow import + parameter matrix.** `services/comfyWorkflowParser.ts` injects prompt/seed/steps/cfg/sampler into a user-supplied ComfyUI graph (`injectWorkflowParameters`) — the escape hatch for split-encoder checkpoints (Flux, SD3) the default workflow deliberately doesn't cover (see the ComfyUI entry under Phase 4 above). `validateWorkflowOnComfy()` submits the injected graph to the real `/prompt` endpoint and requires `node_errors` to be empty before treating an import as valid, surfacing ComfyUI's own per-node error verbatim — same contract shape live-verified in Phase 4. Note this "validation" is a real submission: a `node_errors: {}` response means the job is now genuinely queued and executing on ComfyUI, not merely schema-checked; there's no dry-run endpoint to use instead. `services/matrixGenerator.ts` computes the full Cartesian product across prompts × models × LoRA weights × CFG scales × samplers for `batchQueue.ts`, with a `checkJobCountGate()` requiring confirmation past 25 jobs (a matrix can silently produce hours of GPU time otherwise). 40 tests. Neither service has a UI wired up yet — both are called only from tests so far.
- [x] **Refiner model-aware weight controls.** T5/LLM-conditioned models (Flux, Midjourney v6+, Imagen 3, DALL·E 3) parse natural language and degrade under `(token:1.3)` weighting syntax; classic diffusion engines (SD 1.5, SDXL, Pony, Illustrious, A1111/Forge) depend on it. `ModelProfile` (`constants/modelProfiles.ts`) gained `supportsTokenWeighting`/`weightSyntax`/`minWeight`/`maxWeight`/`weightStep` fields (extending the existing registry rather than adding a parallel one), and `lookupModelProfile()`'s return type widened from `{format, rules}` to the full profile — verified both real callers only destructure `.format`/`.rules`, so this is backward compatible. `serializeModifierToken()` applies the weight syntax per model. `RefinerModifierControls.tsx` shows a weight slider next to ~26 modifier fields, gated on `supportsTokenWeighting`.
  - **Shipped inert, then fixed 2026-07-29.** The slider changed a number in React state and persisted it to settings — but `modifierWeights` was never threaded into `enhancePromptStream`, and `serializeModifierToken()` had zero callers anywhere outside its own file and tests. Dragging the slider to 1.35 and generating produced byte-identical output to weight 1.0 — a UI control with no functional effect, same shape of defect as the ComfyUI empty-checkpoint bug and the dispatcher's `"dispatched (stub)"` string earlier in this same effort. Every WS5 test at the time exercised `serializeModifierToken()` in isolation; none exercised the real Refiner → `enhancePromptStream` → `buildContextForEnhancer` path where the gap actually was. Fixed: `buildContextForEnhancer` and `enhancePromptStream` both take `modifierWeights` as optional trailing params (defaulting to `{}`, so all pre-existing call sites and tests are unaffected), and `RefinerPage.tsx` now passes its live slider state through instead of only persisting it. 6 new tests, 2 of which exercise `enhancePromptStream` itself rather than the isolated function — the class of test that was missing. **Separate, still-open gap found in the same trace:** `modifiers.realism` has a weight slider in the UI, but `buildContextForEnhancer` never read `modifiers.realism` at all, to begin with — the realism modifier's value doesn't reach the prompt, weighted or not.
- [x] **Server modularization.** `server.ts` route handlers extracted into `routes/reachRoutes.ts`, `routes/searchRoutes.ts`, `routes/mcpRoutes.ts`, `routes/localModelRoutes.ts`, `routes/topazRoutes.ts` — one router per commit, full paths and middleware order (rate limiter → validate → handler) preserved exactly, spot-verified against the pre-extraction code. `server.ts` down from 1,792 to ~1,150 lines. **CSP was deliberately left untouched** — the switch to enforced `Content-Security-Policy` is still blocked on 5 unrun manual verifications (ISSUE-30), not something a route-extraction pass should jump ahead of.

### Phase 6 — Assistant Responsiveness + ComfyUI/A1111 Studio Pages ✅

Three independently-shipped improvements shipped 2026-07-29, each targeted at a concrete bottleneck.

- [x] **Assistant reply latency.** `buildKnowledgeContextBlock()` in `services/assistantService.ts` ran sequential DB round-trips before every reply: an `await`-in-a-for-loop over `trackAccess()` calls (up to 8 sequential round-trips), and two independent searches (`knowledgeService.search` + `memoryTierService.searchAll`) run one after the other. Both parallelized with `Promise.all` — the loop became `Promise.all(results.map(r => trackAccess(r.ref).catch(...)))`, and the two search sections became independent async functions run via `Promise.all`. 4 new tests in `services/assistantService.test.ts`. The function's external signature is unchanged.
- [x] **Voice turn-taking configurability.** `services/turnManager.ts` hardcoded `silenceTimeoutMs = 800` — the duration of silence required before the assistant starts processing speech. Added `voiceSilenceTimeoutMs` to `LLMSettings` (default 800ms, preserved), `resolveVoiceSilenceTimeoutMs()` resolver in `services/liveAssistantService.ts`, and a range slider (`300–2000ms`, step 100) in `components/settings/AssistantSection.tsx`. Field added to the `SetupPage.tsx` persistence allow-list. 3 new tests in `services/liveAssistantService.test.ts`, 3 in `components/settings/AssistantSection.test.tsx`.
- [x] **Chat streaming smoothness.** Two fixes applied together:
  1. **Memoized `MessageBubble`** — extracted the inline message rendering from `components/LLMChatPanel.tsx` into `components/MessageBubble.tsx` wrapped in `React.memo`. Because `setMessages` builds the next array as `[...prev]` with only the streaming element replaced, every other element keeps the same object reference — `React.memo`'s default shallow comparison skips those bubbles entirely. 4 new tests in `components/MessageBubble.test.tsx`.
  2. **Animation-frame chunk batching** — `components/chatStreamThrottle.ts` exports `createChunkFlusher()` which batches rapid successive text chunks into one `setMessages` call per animation frame instead of one per chunk. Wired into `LLMChatPanel.tsx`'s streaming loop; `flushNow()` called on `turn_end` and in the `catch` block to preserve partial responses on error. 3 new tests in `components/LLMChatPanel.streamThrottle.test.tsx`.

**ComfyUI / A1111 Dedicated Studio Pages** (same date, 5 tasks):

- [x] **Backend adapter fixes.** `services/a1111Service.ts` now sends `override_settings.sd_model_checkpoint` when `params.model` is provided (previously dropped it). `constants/comfyWorkflows.ts`'s `createDefaultWorkflow()` accepts a `samplerName` param (defaulting to `'euler'`), threaded through from `services/comfyService.ts`. 4 new tests across both service test files.
- [x] **Checkpoint persistence.** `comfyModel` and `a1111Model` fields added to `LLMSettings` (default `''`), with defaults, hydration, and persistence allow-list wiring. 3 new tests in `utils/settingsStorage.test.ts`.
- [x] **`useLocalGenerationStudio` hook.** Async state machine in `hooks/useLocalGenerationStudio.ts` — availability check, model list refresh, generate with `AbortController` cancel support, and gallery ingestion via `addItemToGallery`. 7 tests in `hooks/useLocalGenerationStudio.test.ts`.
- [x] **`LocalGenerationStudioPage` component.** `components/LocalGenerationStudioPage.tsx` — one component mounted twice (once per `backendId`). Two-column layout: params panel (checkpoint dropdown, prompt, negative prompt, dimensions, steps, CFG, sampler, seed, Generate/Cancel) + result panel (preview image, seed, gallery link). Connected-status badge, model persistence via `useSettings()`. 5 tests in `components/LocalGenerationStudioPage.test.tsx`.
- [x] **Navigation wiring.** `comfy_studio` and `a1111_studio` added to `ActiveTab` (`types.ts`), `PAGES` (`services/assistantTools.ts`), `utilityItems` (`components/Header.tsx`), `commandRegistry.ts` (`constants/`), and the render/title switches in `components/App.tsx`.

**Gated at design review, not built:** a gallery visual-similarity search workstream (blocked on an unverified assumption — whether Ollama's `/api/embed` even accepts image input; the contract verified live in Phase 4 is text-only) and a local vault-folder extension/plugin hub (blocked on a real threat model — a Web Worker is not a security boundary; it shares the page origin, has a global `fetch` that a `networkAllowlist` on a bridge API doesn't constrain, and reaches the same IndexedDB as the vault index). Neither has any code in the repo.

### Phase 7 — Kollektiv Adaptation: Durable Generation Record, Vault Wake, Adapter-Free Transport ✅

Twelve work packages (2026-08-05/06), originally tracked in `docs/plans/2026-08-05-kollektiv-adaptation-roadmap.md` (deleted once implemented and folded in here). Thesis: the prompt-authoring layer (Crafter, Refiner, the vault) is the durable asset; generation services are commodity and churn. Landed as 12 commits, then code-reviewed in the same session — the review found 9 of 11 critical findings were the exact "built but not wired" shape already tracked earlier in this document (relevant new-work-had-zero-callers pattern), not implementation bugs in the new code itself. All findings below were fixed and re-verified (typecheck clean, 1244 tests passing, up from the 1216 baseline) before this entry was written — the bullets describe the fixed state, with the as-shipped defect noted where it's a useful lesson.

- [x] **WP1 — Wake the vault.** `initSearchIndex()` and `indexWikilinksIntoGraph()` (`utils/obsidianStorage.ts`) had zero callers — wired into `useBootSequence.ts` right after `initObsidianVault()` succeeds. `syncAgentMemoryToVault()` (`utils/memoryStorage.ts`) also shipped uncalled: the boot code's own gate checked `getAgentMemoryBlock()`, which only that same function ever sets — a call that can never fire on a fresh load. Fixed to gate on `getMemoriesSync().length > 0` instead. Separately, `hydrateKnowledgeGraph()` (`services/tools/graphHydration.ts`) calls `relationshipGraph.clear()` on every call but never re-ran wikilink extraction, so the first `VaultMapPanel` open (or any graph-tool call) silently wiped the edges WP1 had just added at boot — fixed by re-running `indexWikilinksIntoGraph()` at the end of every hydration, not just once. See [OBSIDIAN.md](../03_KNOWLEDGE_ENGINE/OBSIDIAN.md).
- [x] **WP2 — Manifest `schemaVersion`.** `stampSchemaVersion()` (`utils/manifestStore.ts`) added to all six manifests via object spread. Absent means v1; readers tolerate both, writers emit v2. Purely additive — verified no reader changed.
- [x] **WP3 — `Generation` record.** New `utils/generationStorage.ts` (follows the `presetStorage.ts` `loadManifestSafe` pattern) makes every local generation's full params (model, sampler, seed, steps, cfg) reproducible from the vault instead of discarded at ingest. **Shipped with the API built but disconnected from every actual generation path** — `useLocalGenerationStudio.ts`, `useGenerateLoop.ts`, and all three generation-producing tools in `assistantTools.ts` (`generate_image`, `generate_and_ingest`, `save_to_gallery`) still computed params and dropped them, same defect the roadmap was written to fix. Wired at all five sites in the same session; `addItemToGallery()`'s options-bag param (added for `generationId`) is used at the sites that need it, left positional at the three manual/non-generation call sites (`ImageGallery.tsx`, `ComposerPage.tsx`, `knowledgeService.ts`) that have no params to attach. `utils/integrity.ts` now tracks `generations.json` and reports `%` of gallery items with no linked generation via `getGenerationCoverageReport()`, surfaced in the Settings → App integrity-check feedback.
- [x] **WP4 — Surface it.** `ItemDetailView.tsx`'s metadata block reads model/sampler/steps/cfg/seed/backend from the linked `Generation`. "Load these settings" originally routed by sniffing `generation.params.model` for the substring `"comfy"` (wrong — `generation.backendId` already carries the exact `'comfy'`/`'a1111'` value) and emitted `appEventBus`'s `navigate` event, which only carries a tab id, not a params payload — so nothing was actually loaded despite the toast claiming otherwise. Fixed: a module-level pending-params handoff (`setPendingStudioParams`/`consumePendingStudioParams` in `useLocalGenerationStudio.ts`, mirroring the existing `_agentMemoryBlock` pattern) stashes params before navigating; `LocalGenerationStudioPage.tsx` consumes them on mount. Also fixed a stale-promise race in the generation-loading `useEffect` (no cancellation guard meant a fast arrow-key navigation could paint a previous item's generation onto a legacy item).
- [x] **WP5 — Search coverage.** `indexGalleryAndPrompts()` feeds gallery items and saved prompts into the BM25 index as `gallery://<id>`/`prompt://<id>` pseudo-documents. **Shipped as a regression, not a no-op:** `searchNotes()`'s ranked-result loop called `readFile()` on every hit including these pseudo-paths, which always returns `null` for them — so gallery/prompt hits were silently dropped, and if the top-ranked set was entirely non-vault docs, the function returned `[]` directly instead of falling through to the substring-scan fallback, meaning a query that worked before this WP could return zero results after it. Fixed by resolving pseudo-path content from the gallery/prompt manifests (batch-loaded once per call) instead of `readFile()`. See [OBSIDIAN.md](../03_KNOWLEDGE_ENGINE/OBSIDIAN.md).
- [x] **WP6 — MCP generation bridge.** `services/mcpGenerationBridge.ts` turns an MCP tool call that returns image/video content into a `GalleryItem` + `Generation` (`backendId: 'mcp:<server>/<tool>'`) with zero adapter code per service (per D5: transport is config, not hand-written adapters). **Shipped disconnected** — `bridgeMcpGeneration()` had no caller; the commit message claimed it was wired into `mcpAssistantTools.ts` but that file was untouched. Fixed: `mcpAssistantTools.ts`'s tool `execute()` now checks `detectMcpMedia()` on every MCP tool result and bridges it when present. Also tightened `detectMcpMedia()` to only trust explicit typed content blocks (`{type:'image'|'video'}`) or a whole-string data URL — the original also regex-scanned any tool's freeform text for embedded base64/video-URL substrings, which would bridge an unrelated tool's incidental output (a doc link, a log line) as if it were generated media. `params` now stores the real invocation args (`GenerateParams.raw`) instead of always zeroing width/height/steps/cfg. **Not done:** registering MCP generators in `listBackends()` — deliberately skipped, since nothing in the app currently reads `listBackends()` at all (confirmed zero consumers outside its own test); building that registration with no surface to expose it to would be speculative scaffolding.
- [x] **WP7 — Profile-driven external targets.** `constants/modelProfiles.ts` extended with current external targets (data-only change). `services/externalCopyRecorder.ts`'s `recordExternalCopy()` records a `Generation` (`backendId: 'external:<name>'`, empty `resultItemIds`) when a prompt is copied out for a service with no API. **Shipped with zero callers** — wired into `RefinerPage.tsx`'s `handleCopySuggestionText`, the actual copy-to-clipboard action, in the same session.
- [x] **WP8 — `RefinerPreset` as creative asset.** Five optional fields (`kind`, `tags`, `freeform`, `previewItemId`, `useCount`) on the existing preset type. No editor, no asset library, no new store — the only WP8 shipped clean on the first pass.
- [x] **WP9 — Script → cast and locations.** `services/tools/storyAssetExtractor.ts`, an `extract_story_assets` assistant tool for Fountain-formatted scripts. **Shipped with two real defects**, not just disconnection: (1) any ALL-CAPS line ≥3 chars was treated as a character cue, filtering only 9 known transition words — Fountain slugs like `MONTAGE`/`SUPER:` would silently become character notes, the exact "must degrade visibly, not emit garbage" failure the roadmap explicitly called out to avoid; (2) the tool never created the `RefinerPreset` half of the required "two artifacts per entity" at all, despite its own description claiming it did. Rewritten: a cue must recur ≥2 times to count (rejected candidates are reported in the tool's own output, not silently dropped), a single vault walk tracks which scene each cue follows so character/location notes cross-link by shared scenes (previously wikilinks pointed only at scene headings, which were never their own notes — always dangling), and each character/location now gets both the vault note and a `RefinerPreset` (modifiers seeded from `HAIR_STYLES`/`EYE_COLORS`/`CLOTHING_STYLES` as placeholders, not fabricated traits).
- [x] **WP10 — Implicit signals → composite score.** `services/generationSignals.ts` computes a 0–1 composite score per `Generation` from survival/publish/param-reuse/naming/notes/tags/pinned signals (D6: aggregates only, never a per-image verdict). **Shipped with the scoring pipeline itself unreachable** (zero callers anywhere, confirmed by grep) and two silent bugs that would have produced wrong scores even if it had been called: `item.isPinned` checked a field that doesn't exist on `GalleryItem` (pinning is `pinnedIds[]` in the gallery manifest), and the `paramReuse` weight was declared but never read in `computeScore`, capping the achievable score at 0.80 of 1.00. Fixed: pinned-state now reads `loadPinnedItemIds()`; param reuse is detected via `(backendId, resolvedSeed)` recurrence across generations (the cheapest signal derivable from existing data, since WP4's "Load these settings" is a live UI action with nothing persisted to log a reuse event). Exposed via a new `score_generations` assistant tool (`services/tools/generationSignalsTool.ts`) rather than a background job — per D6, the assistant proposes the write.
...
| `a1111_studio` | `LocalGenerationStudioPage` | Dedicated A1111/Forge Neo generation page — same component as ComfyUI Studio, parameterized by `backendId`. Added 2026-07-29. |
...
**Recurred at scale in Phase 7 (2026-08-05/06):** a 12-work-package roadmap landed with 9 of 11 review-critical findings being this exact pattern — new code with a real caller graph gap, not a bug in the code itself (`syncAgentMemoryToVault`, five `Generation`-record ingest sites, the MCP generation bridge, `generationSignals.ts`'s entire scoring pipeline, and more). See the Phase 7 section above for the full list — small diffs that add a new module but never thread it into an existing call path are apparently this codebase's most common failure mode, worth checking for specifically in review rather than trusting "the commit message says it's wired."
