...
import { securityHeaders, authRateLimiter, searchRateLimiter, reachRateLimiter, twitterReachRateLimiter, corsOptions } from "./src/middleware/security";
...
async function startServer() {
  ...
  app.use("/comfy-local", async (req, res) => {
    try {
      const subPath = req.url;
  ...
  app.use("/a1111-local", async (req, res) => {
    try {
      const subPath = req.url;
  ...
  // Each route is a thin adapter: validate -> call the matching services/<x>Service.ts -> JSON response.
  // Routed server-side (not client-direct fetch) so third-party hosts never need CSP connect-src
  // entries and any optional API keys (GITHUB_TOKEN, EXA_API_KEY) stay off the client.

  // Fetch and parse an RSS/Atom feed
  app.post("/api/reach/rss", reachRateLimiter, validate(RssRequestSchema), async (req, res) => {
  ...
  app.post("/api/reach/github", reachRateLimiter, validate(GithubRequestSchema), async (req, res) => {
  ...
  app.post("/api/reach/exa", reachRateLimiter, validate(ExaSearchRequestSchema), async (req, res) => {
    try {
  ...
  app.post("/api/reach/reddit", reachRateLimiter, validate(RedditRequestSchema), async (req, res) => {
    const body = req.body as
  ...
  app.post("/api/reach/youtube-transcript", reachRateLimiter, validate(YoutubeTranscriptRequestSchema), async (req, res) => {
  ...
  app.post("/api/reach/twitter", twitterReachRateLimiter, validate(TwitterRequestSchema), async (req, res) => {
