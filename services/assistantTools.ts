...
export const ASSISTANT_TOOLS: AssistantTool[] = [
    ...
    // ─── Obsidian Second Brain (Wave 1 — search/read/write/list) ──
    ...
    },
    {
        ...
        execute: async ({ prefix }) => {
            ...
            return JSON.stringify(paths.map(p => ({ path: p, title: p.replace(/\.md$/, '').split('/').pop() })));
        },
    },
    {
    ...
    // ─── Obsidian Second Brain (Wave 2-3 — append/delete/patch/frontmatter/open) ──
    ...
    },
    {
        ...
        execute: async ({ path, key, value }) => {
            if (!isObsidianConnected()) return 'Error: Obsidian vault is not connected.';
            const oldFm = await getNote(String(path)).then(n => n?.frontmatter?.[String(key)]);
