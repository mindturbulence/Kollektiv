...
const ImagePreview: React.FC<ImagePreviewProps> = ({ url, onRemove }) => {
    ...
    return (
        <div className="relative group aspect-square bg-transparent overflow-hidden">
            {hasError || !blobUrl ? (
            ...
            <button onClick={onRemove} className="btn btn-sm btn-square btn-error absolute top-2 right-2 opacity-0 group-hover:opacity-100">
...
export const ImageManagementModal: React.FC<ImageManagementModalProps> = ({ isOpen, onClose, item, onSave }) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-xl z-[110] flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-base-100/40 rounded-none w-full max-w-4xl mx-auto flex flex-col max-h-[90vh] overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-10 flex-grow overflow-y-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-px bg-base-100/40 backdrop-blur-xl mb-8">
            ...
            <button 
                ...
                className="aspect-square bg-transparent flex flex-col items-center justify-center text-base-content/40 hover:text-primary hover:bg-primary/10 transition-all group"
            >
        ...
        <footer className="p-4 border-t border-base-300 flex justify-end gap-2 bg-transparent">
          ...
          <button onClick={handleSave} className="btn btn-primary rounded-none uppercase font-black text-[10px] tracking-widest px-8">Commit Artifacts</button>
        </footer>
      </div>
    </div>
  );
