...
const CheatsheetDetailView: React.FC<CheatsheetDetailViewProps> = ({
  ...
  return (
    <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm animate-fade-in flex items-center justify-center p-2 lg:p-4 overflow-hidden" onClick={onClose}>
        <div className="w-full h-full bg-transparent rounded-none border border-base-300 flex flex-col overflow-hidden relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-grow flex flex-col lg:flex-row overflow-hidden">
                    ...
                    <div className="max-w-screen-xl mx-auto w-full space-y-12">
                        ...
                        <div className="space-y-6">
                            ...
                            {item.imageUrls.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-px bg-transparent border border-base-300">{item.imageUrls.map((url, idx) => (<div key={idx} className="aspect-square bg-transparent overflow-hidden"><ArtifactPreview url={url} /></div>))}</div>
                ...
                <aside className="w-full lg:w-96 flex-shrink-0 bg-transparent border-l border-base-300 flex flex-col">
                    <div className="p-8 space-y-8 flex-grow overflow-y-auto"><div className="space-y-4"><span className="text-[10px] font-black uppercase tracking-widest text-base-content/30">Registry Info</span><div className="p-4 bg-transparent border border-base-300 rounded-none space-y-4"><div><h4 className="text-[9px] font-black uppercase text-primary/60 mb-1">Frequency</h4><p className="text-sm font-mono font-bold">{item.imageUrls.length} Samples</p></div></div></div></div>
