...
const ArtifactPreview: React.FC<{ url: string }> = ({ url }) => {
    ...
    if (!blobUrl) return <div className="aspect-square bg-transparent animate-pulse" />;
...
const CheatsheetDetailView: React.FC<CheatsheetDetailViewProps> = ({
  ...
  return (
    <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm animate-fade-in flex items-center justify-center p-2 lg:p-4 overflow-hidden" onClick={onClose}>
        <div className="w-full h-full bg-base-100/40 backdrop-blur-xl rounded-none flex flex-col overflow-hidden relative" onClick={e => e.stopPropagation()}>
            <header className="flex-shrink-0 px-8 h-16 border-b border-base-300 bg-transparent flex justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="join bg-base-100/40 backdrop-blur-xl">
            ...
            <div className="flex-grow flex flex-col lg:flex-row overflow-hidden">
                <main className="flex-1 flex flex-col overflow-y-auto custom-scrollbar bg-transparent p-8 lg:p-12">
                    <div className="max-w-screen-xl mx-auto w-full space-y-12">
                        ...
                        <div className="space-y-6">
                            ...
                            {item.imageUrls.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-px bg-base-100/40 backdrop-blur-xl">{item.imageUrls.map((url, idx) => (<div key={idx} className="aspect-square bg-transparent overflow-hidden"><ArtifactPreview url={url} /></div>))}</div>
                ...
                <aside className="w-full lg:w-96 flex-shrink-0 bg-base-100/40 backdrop-blur-xl flex flex-col">
                    <div className="p-8 space-y-8 flex-grow overflow-y-auto"><div className="space-y-4"><span className="text-[10px] font-black uppercase tracking-widest text-base-content/30">Registry Info</span><div className="p-4 bg-base-100/40 backdrop-blur-xl rounded-none space-y-4"><div><h4 className="text-[9px] font-black uppercase text-primary/60 mb-1">Frequency</h4><p className="text-sm font-mono font-bold">{item.imageUrls.length} Samples</p></div></div></div></div>
                    <footer className="border-t border-base-300 flex bg-transparent p-0 overflow-hidden flex-shrink-0"><button onClick={() => { onInject(item); onClose(); }} className="btn btn-primary flex-1 rounded-none font-black text-[10px] tracking-[0.2em] transition-colors hover:brightness-110"><SparklesIcon className="w-5 h-5 mr-3 inline-block"/>INJECT</button></footer>
                </aside>
            </div>
