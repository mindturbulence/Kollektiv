...
export const PromptAnalyzer: React.FC<PromptAnalyzerProps> = ({
    ...
    const filteredSegments = useMemo(() => {
        return modifierSegments.filter(seg => {
            const val = (seg.value || "").trim().toLowerCase();
            return val !== '' && val !== 'none' && val !== 'n/a' && val !== 'null' && val !== 'standard' && val !== 'default' && val !== 'undefined' && val !== '[none]' && val !== 'not specified';
        });
    }, [modifierSegments]);
    ...
    // --- Reconstruction Manual Sync ---
    const handleSync = async () => {
        if (!subjectPrompt && modifierSegments.length === 0) return;
        setIsBusy(true);
        ...
        try {
            ...
            const res = await reconstructPrompt(components, settings);
            setReconstructedPrompt(cleanLLMResponse(res));
        } catch (err) {
            showGlobalFeedback('Synchronization failed.', true);
        } finally {
            setIsBusy(false);
        }
    };
    ...
    const handleDissect = async (input?: string) => {
        ...
        try {
            ...
            Object.entries(result.modifiers).forEach(([key, value]) => {
                ...
                console.log('Adding modifier:', key, '=', valStr);
                newSegments.push({
            ...
            const suggestedValues = (result.categorizedParameters || []).map(p => p.value.toLowerCase()).filter(Boolean);
            ...
            const finalConstant = [result.constantModifier, ...unmappedItems]
                .filter(Boolean)
                .join(', ');

            // Filter suggested parameters - only keep ones with meaningful values
            // Also check if there's an existing modifier with the same key - if so, keep only the one with value
            const validParams: { label: string, value: string }[] = [];
            const seenKeys = new Set<string>();
            
            (result.categorizedParameters || []).forEach(p => {
                ...
                const existingModifier = newSegments.find(s => s.key.toLowerCase() === key);
                if (existingModifier && existingModifier.value.trim()) {
                    // Modifier already has value, skip this suggested param
                    return;
                }
                
                // Check if we already added this suggested param
                if (seenKeys.has(key)) {
                    return;
                }
                    ...
                    <div className="flex flex-col h-full w-full overflow-hidden relative z-10 bg-base-100/40 backdrop-blur-xl panel-transparent">
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar">
                            {hasBreakdown ? (
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                                            {filteredSegments.map(seg => (
                                                <div key={seg.id} className="flex flex-col space-y-2 animate-fade-in group">
                                                            ...
                                                            {availableModifierKeys.filter(k => !modifierSegments.some(s => s.key === k)).length === 0 ? (
                                                                ...
                                                                availableModifierKeys.filter(k => !modifierSegments.some(s => s.key === k)).map(key => (
                                                                    <button
                                                                        ...
                                                                        type="button"
                                                                        onClick={() => {
                                                                            console.log('Adding modifier:', key);
                                                                            setModifierSegments((prev) => {
                                                                                ...
                                                                                console.log('New modifier list:', [...prev, newSegment]);
                                        ...
                                        {/* Custom Parameters Section - Auto-generated from LLM, editable */}
                                ...
                                <div className="h-full flex flex-col items-stretch animate-fade-in p-6">
                                    <textarea
                                        value={promptInput}
                                        onChange={(e) => setPromptInput(e.target.value)}
                                        placeholder="Enter complex prompt sequence to deconstruct into atomic nodes..."
                                        className="w-full h-full bg-base-100/10 border border-base-content/10 focus:border-primary/50 focus:outline-none p-6 font-nunito font-bold text-lg leading-relaxed resize-none transition-all placeholder:text-base-content/10 rounded-none shadow-inner custom-scrollbar"
                                    />
                                </div>
                            )}
                        </div>
                                ...
                                {!hasBreakdown ? (
                                    <>
                                        ...
                                        <button
                                            ...
                                            className="btn btn-sm btn-primary h-full flex-1 rounded-none font-bold text-[11px] tracking-widest uppercase btn-snake font-display"
                                        >
                                            <span /><span /><span /><span />
                                            ANALYZE
                                        </button>
                                    ...
                                    <>
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                console.log('subjectPrompt:', subjectPrompt);
                                                console.log('modifierSegments:', modifierSegments);
                                                console.log('customParameters:', customParameters);
                                                console.log('constantModifier:', constantModifier);
                                                
                                                const modifiedText = reconstructFromParameters();
                                                console.log('reconstructed text:', modifiedText);
                                                ...
                                                // If there are modifications beyond the original, apply light rewriting
                                                ...
                                                if (hasModifications) {
                                                    ...
                                                    // No modifications, just show original
                                                    console.log('No modifications, showing original');
                    ...
                    <div className="flex flex-col h-full w-full overflow-hidden relative z-10 bg-base-100/40 backdrop-blur-xl panel-transparent">
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar p-6">
                            <div className="h-full flex flex-col overflow-hidden">
                                ...
                                <div className="flex-1 flex flex-col min-h-0 overflow-hidden pt-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-primary/60 mb-2">Modified Prompt</label>
                                    <div className="flex-1 relative group flex flex-col p-4 bg-base-100/10 border border-primary/20 overflow-y-auto">
                                        {isRewriting ? (
                                            ...
                                            <blockquote className="text-base md:text-lg font-nunito font-bold italic leading-relaxed tracking-tight text-base-content selection-bg-primary/30">
                                                "{modifiedPrompt || 'Modify parameters and press RE-WRITE...'}"
                                            </blockquote>
                                        )}
                                    </div>
                                </div>
                        ...
                        <footer className="h-14 flex items-stretch border-t border-base-content/5 bg-base-100/10 backdrop-blur-md p-1.5 gap-1.5 panel-footer">
                            <div className="flex flex-1 w-full h-full items-stretch gap-1.5">
                                ...
                                <button
                                    onClick={() => {
                                        if (modifiedPrompt && onMapToRefiner) {
