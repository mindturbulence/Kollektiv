export const handleGeminiError = (error: unknown, context: string): Error => {
  ...
  if (error instanceof Error) {
    ...
    if (lowerCaseMessage.includes("failed to fetch") || lowerCaseMessage.includes("networkerror")) {
        errorMessage = `A network error occurred while ${context}. If using local Ollama, ensure OLLAMA_ORIGINS="*" is set and use HTTP if your site is not on HTTPS. If your site IS on HTTPS, browsers block local HTTP calls (Mixed Content). Please ensure your API/OpenClaw/Ollama is reachable.`;
